...
| [Seerr](https://github.com/royal32/seerr)                          | Manages requests for your media library                                                                                                                       | Pulled from [royal32/seerr](https://github.com/royal32/seerr)                                                                | `seerr.${TAILNET_DOMAIN}` |
...
| `ARR_PUBLIC_QUALITY_PROFILE`   | Managed Sonarr/Radarr quality profile name. The reconciler keeps this as the only quality profile, prefers 4K, falls back to 1080p or lower, disables CAM quality, and prefers English-language releases without blocking other languages. | `Public 4K Preferred`                            |
| `ARR_MAX_GB_PER_HOUR`          | Default maximum media size in GB per hour for managed quality definitions. Converted to MB/minute before calling Sonarr/Radarr.                                                                         | `8`                                              |
| `SONARR_QUALITY_PROFILE`       | Managed Sonarr quality profile name; defaults to `ARR_PUBLIC_QUALITY_PROFILE`.                                                                                                                         | `Public 4K Preferred`                            |
...
| `SEERR_RADARR_PROFILE`         | Radarr quality profile name selected automatically for Seerr movie requests                                                                                                                            | `RADARR_QUALITY_PROFILE`                         |
| `SEERR_RADARR_MINIMUM_AVAILABILITY` | Radarr minimum availability used for Seerr movie requests                                                                                                                                        | `released`                                       |
| `SEERR_SONARR_PROFILE`         | Sonarr quality profile name selected automatically for Seerr series requests                                                                                                                           | `SONARR_QUALITY_PROFILE`                         |
...
It also creates a `Public 4K Preferred` quality profile in both apps and removes every other quality profile after existing movies/series are reassigned to the managed profile. That profile allows every non-CAM quality, prefers 2160p when available, falls back to 1080p, then accepts lower qualities when neither is available. The reconciler adds an `English Preferred` custom format with a positive score and keeps the minimum custom format score at zero, so English-language releases are preferred when available without blocking foreign-language media. The reconciler caps managed quality definitions at `ARR_MAX_GB_PER_HOUR` converted to the Arr API's MB/minute units.
...
Ofelia remains running and starts both containers at `02:58` each day. After Profilarr becomes healthy, Ofelia keeps both containers available for `PROFILARR_RUN_WINDOW`, then stops them. The default 30-minute window ensures Profilarr is running across the 3am trigger and gives scheduled jobs until approximately 3:30am to finish.
