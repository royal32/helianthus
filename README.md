...
| `PROFILARR_TRIGGER_GRACE`      | Time Ofelia waits after Profilarr becomes healthy before checking for completed jobs, ensuring the 3am trigger has passed.                                                                               | `3m`                                             |
| `PROFILARR_JOB_WINDOW_SECONDS` | How far ahead Ofelia considers queued Profilarr jobs part of the daily window. This permits jobs staggered across the first 20 minutes after 3am.                                                        | `1200`                                           |
| `PROFILARR_RUN_WINDOW`         | Hard timeout for the daily Profilarr window after it becomes healthy. Profilarr normally stops earlier when no active or in-window jobs remain.                                                         | `30m`                                            |
...
Ofelia remains running and starts both containers at `02:58` each day. After Profilarr becomes healthy, Ofelia waits through the 3am trigger and polls Profilarr's supported status API. It keeps Profilarr running for queued jobs scheduled inside the next `PROFILARR_JOB_WINDOW_SECONDS`, then stops once there are no active jobs or jobs remaining inside that window. Three consecutive idle checks prevent shutdown between serial jobs. `PROFILARR_RUN_WINDOW` remains a hard timeout; if it interrupts a running job, Profilarr resets that job to queued and retries it on its next startup.

Enable the desired schedules in Profilarr itself and target them between 3am and approximately 3:20am. Ofelia controls when Profilarr is available but does not create Profilarr jobs. Change `PROFILARR_START_SCHEDULE`, `PROFILARR_TRIGGER_GRACE`, `PROFILARR_JOB_WINDOW_SECONDS`, or `PROFILARR_RUN_WINDOW` in `.env` to adjust the window.
